...
The mathematical theory that underlies the rest of the paper is basic algebraic topology, but some prefatory remarks are in order about why we chose certain abstractions to work with and not others.
An introduction to algebraic topology might start from the definition simplicial or cubical complexes.
...
By contrast, one could instead consider the category of CW complexes, which has certain theoretical and computational advantages and which includes both simplicial and cubical complexes.
...
Here, we consider \emph{polytopal complexes}, which includes the simplicial and cubical cases but is less general than the full CW category.
...
The advantage of polytopal complexes is that we can explicitly represent non-simplicial intermediate states.
For example, we might merge several triangles of a mesh into a polygon which we then re-triangulate in a different way.
...
The $k$-\emph{skeleton} is the set of all $k$-dimensional cells of $\Omega$, which we write as $\text{skel}_k(\Omega)$.
We moreover assume that, for any $k$-cell $\sigma$ and $k - 1$-cell $\tau$, there is an \emph{incidence number} between them, which we write as $[\sigma, \tau]$, such that
\begin{equation}
...
A \emph{chain} $C$ is a formal integer linear combination of $k$-cells:
\begin{equation}
    C = c_1\sigma_1 + \ldots + c_m\sigma_m
\end{equation}
where the $c_i$ are integer coefficients and $\sigma_i$ are $k$-cells.
We write the space of all $k$-chains on a complex $\Omega$ as $\mathscr{C}^k(\Omega)$ or just $\mathscr{C}^k$ where the underlying space is clear from context.
...
\begin{equation}
    \partial_k\sigma = \sum_{\tau \in \text{skel}_{k - 1}(\Omega)}[\sigma, \tau]\tau
\end{equation}
...
\begin{equation}
    ...
    \label{eq:ddzero}
\end{equation}
This equation is a consequence of the formal definition of incidence numbers on a CW complex \cite{hatcher2002algebraic}.
Alternatively, if we choose to define a polytopal complex by its boundary operators, we can take the last equation to be a definition or precondition.
