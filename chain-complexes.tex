...
Other domains of science and engineering have seized on the idea of using linear algebra as the \emph{lingua franca} for building applications as well.
...
Many readers will scratch their heads and wonder what on earth we mean by multiplying a number and a triangle.
...
First, we briefly review the theory of differential forms.
Let $\Omega$ be a manifold, and $u$ a completely antisymmetric rank-$k$ tensor field on $\Omega$.
...
The exterior derivative can be defined axiomatically by requiring that (1) if $f$ is a scalar field or 0-form, then its exterior derivative is equal to its differential; (2) if $f$ is a scalar field, then $d(df) = 0$; and (3) $d(u \wedge v) = du\wedge v + dv\wedge u$.
From these properties, we can deduce that $d(du) = 0$ for any form $\omega$.

Most important for us is the generalized fundamental theorem of calculus or Stokes theorem.
Given a $k$-form $u$ and a smooth, oriented $k$-dimensional submanifold $M$ of $\Omega$, we can define the integral of $u$ on $M$.
...
A $k$-\emph{current} is a continuous linear functional on $\bigwedge^k\Omega$.
...
We can think of a submanifold $M$ as defining a bounded linear functional on $\bigwedge^k\Omega$ by integration:
\begin{equation}
    \langle M, u\rangle \equiv \int_M u.
\end{equation}
(Here we'll abuse notation a little bit by writing both the submanifold and the linear functional as ``$M$''.
...
Given two $k$-dimensional submanifolds $M$ and $N$ and real numbers $\alpha$ and $\beta$, we can define their linear combination $\alpha \cdot M + \beta\cdot N$ by how they act on forms:
\begin{equation}
    \langle \alpha M + \beta N, u\rangle = \alpha\int_M u + \beta\int_N u.
\end{equation}
...
We can then define the \emph{boundary} $\partial J$ of a current $J$ as the formal adjoint of the exterior derivative:
\begin{equation}
    \langle\partial J, u\rangle \equiv \langle J, du\rangle.
\end{equation}
When $J$ is an integral current, this definition agrees with the generalized Stokes theorem.
It also implies that $\partial(\partial J) = 0$, since $d(du) = 0$ for any differential form $u$.
