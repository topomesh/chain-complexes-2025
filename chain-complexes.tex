...
Computing a constrained Delaunay triangulation (CDT), on the other hand, requires both merges and face-splits.
...
Finally, we implemented the tetrahedral mesh transformations described in \cite{misztal2009tetrahedral}.
We tested our implementation as follows:
\begin{enumerate}
    \item Pick some number of vertices $n \ge 4$.
    \item Generate all triangulations $\{T_1, \ldots, T_{C_n}\}$ of the polygon on $n$ vertices ($C_n$ is the $n$th Catalan number).
    \item Repeat for some set number of trials:
        \begin{enumerate}
            \item Pick two triangulations $T$, $T'$ at random.
            \item Generate the suspensions of each triangulation according to equations \eqref{eq:suspension-partial-k} and \eqref{eq:suspension-partial-n+1}.
                Apply a random permutation and sign flip to the boundary matrices of each complex.
                Call the resulting complexes $S$, $S'$.
            \item Compute a chain map from $\partial S$ to $\partial S'$.
            \item \textcolor{red}{Profit???}
        \end{enumerate}
\end{enumerate}
