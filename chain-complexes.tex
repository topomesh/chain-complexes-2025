...
Here we will describe in more detail the underlying theory behind the brief definition we gave of polytopal complexes above.
...
The usual introduction to algebraic topology starts with CW complexes \cite{hatcher2002algebraic}, but understanding how to get from a CW complex to a sequence of boundary operators requires knowledge of homology or degree theory first.
Instead, we will use the theory of \emph{currents} on a manifold, a kind of dual to the theory of differential forms.

Let $\Omega$ be a compact $n$-dimensional differentiable manifold.
We assume as given the theory of differential forms and integration of forms on submanifolds.
We'll write a general $k$-form as $u$ or $v$, $\mathscr F_k\Omega$ for the space of $k$-forms, and $d_k : \mathscr F_k\Omega\to\mathscr F_{k + 1}\Omega$ for the exterior derivative.
For any $k$-form $u$,
\begin{equation}
    d_{k + 1}d_k u = 0.
\end{equation}
Let $M$ be an oriented submanifold with boundary.
We write ``$-M$'' for the same point set but with the opposite orientation, in which case $\int_{-M}u = -\int_Mu$.
...
Any oriented $k$-dimensional submanifold $M$ defines a current by integration.
...
We will also include a \emph{bottom} space $\mathscr{C}^{-1} \cong \mathbb{R}$.
Remembering that the 0-currents are continuous linear functionals on the space of scalar fields, we define the 0-boundary operator as:
\begin{equation}
...
A mesh or polytopal complex is a collection $\{C_{-1}, C_0, \ldots, C_n\}$ such that (1) each $C_k$ is a finite-dimensional $\mathbb{Z}$-submodule of the space of integral $k$-currents on $\Omega$, (2) the complex is closed under taking boundaries, i.e. for any $M$ in $C_k$, there exist integers $c_i$ and currents $N_i$ in $C_{k - 1}$ such that
\begin{equation}
    \partial M = \sum_ic_iN_i,
\end{equation}
and (3) no cell has empty boundary, i.e. for any $M$ in $C_k$, $\partial M \neq 0$.
The last condition is necessary to avoid certain degenerate cases.
