\documentclass[twocolumn,leqno,twocolumn]{article}
...
\usepackage{nicematrix}  % for labelled matrix columns/rows
\NiceMatrixOptions{
code-for-first-row=\scriptscriptstyle,
...
\title{\Large Linear-algebraic representation and transformation of unstructured meshes}
\author{Daniel Shapero\thanks{University of Washington, Seattle, WA. shapero@uw.edu}}
\date{}

\maketitle

\fancyfoot[R]{\scriptsize{Copyright \textcopyright\ 2025\\
Copyright for this paper is retained by authors}}

\begin{abstract} \small\baselineskip=9pt This paper will show some new approaches for implementing common transformations to the connectivity or topology of an unstructured mesh.
...
We show how to implement a few common transformations on the boundary operator representation: merging cells, splitting cells on a new vertex, and splitting a cell on facets.
The source code for these transformations becomes short and easy to test.
\end{abstract}
...
Algorithms for mesh coarsening, on the other hand, apply a sequence of edge or face collapses \cite{gueziec1995surface}.
...
This paper will show three transformations on the linear-algebraic representation of a polygonal mesh: (1) splitting a cell on a vertex, (2) merging adjacent cells, and (3) subdividing a cell along a collection of facets.
...
The key innovation of this paper is using chain complexes to implement certain topological transformations, mainly bistellar flips.
Many previous works have used bistellar flips and a few have used chain complexes.
To our knowledge, no work has combined both.

Nearly all methods for improving the quality of a mesh by transforming its topology use bistellar flips \cite{freitag1997tetrahedral, klingner2008aggressive, misztal2009tetrahedral, hu2018tetrahedral}.
One notable exception is polyhedron reconnection \cite{marot2020reviving}.
These works all assume a half-edge or array-based data structure to describe a simplicial complex.

The closest work to ours is DiCarlo and others \cite{dicarlo2007solid}, which defines the Euler operators and a bisection operator on chain complexes.
The operators defined in that paper work on only two cells at a time; ours operate on an arbitrary number of cells.
Other papers use chain complexes to implement Catmull-Clark subdivision \cite{mueller2017ternary} or to compute arrangements \cite{paoluzzi2020topological}.

Our main results are explicit formulas for certain transformations in arbitrary dimensions.
These are equations \eqref{eq:cone-partial-k} and \eqref{eq:cone-partial-n+1} for vertex splits; \eqref{eq:merge-k+1} for merges; and \eqref{eq:face-split-partial-n} and \eqref{eq:face-split-s-f0} for face splits.
These formulas have not appeared in the meshing literature before.
Equation \eqref{eq:cone-partial-k} has appeared in the homological algebra literature \cite{gelfand1994homological} but not for practical computation.
The GAP and polymake packages both include routines for computing topological cones and performing bistellar moves on simplicial complexes \cite{gawrilow2000polymake, bjorner2000simplicial, effenberger2011simpcomp}.
But both of these packages use array-based data structures rather than the linear-algebraic representation.
...
\begin{figure}[t]
    ...
    \begin{minipage}{0.48\linewidth}
        {\small
            \begin{equation*}
                \partial_1 = \begin{bNiceArray}{ccccc}[small, first-row, last-col]
            ...
            \end{equation*}
            \begin{equation*}
                \partial_2 = \begin{bNiceArray}{cc}[small, first-row, last-col]
                    \mathrm A & \mathrm B &   \\
                    + & - & \mathrm a \\
                    + &   & \mathrm b \\
                    + &   & \mathrm c \\
    ...
    Vertices are labelled with numbers, edges with lower-case letters, and polygons with upper-case letters.}
...
\end{equation}
The incidence number from $\sigma$ to the face $\tau = \langle v_0, \ldots, \hat v_j, \ldots, v_k\rangle$ obtained by removing the $j$th vertex is positive if $j$ is even and negative if $j$ is odd:
\begin{equation}
    [\sigma, \tau] = (-1)^j.
...
\begin{figure}[t]
    ...
    \begin{equation*}
        \partial_1 = \begin{bNiceArray}{cccc}[small, first-row, last-col]
        ...
        \end{bNiceArray}\; ,
        \quad
        \partial_2 = \begin{bNiceArray}{c}[small, first-row, last-col]
            \mathrm A & \\ + & \mathrm a \\ + & \mathrm b \\ + & \mathrm c \\ + & \mathrm d
        \end{bNiceArray}
    \end{equation*}

    \begin{equation*}
        \partial_1' = \begin{bNiceArray}{cccc|cccc}[small, first-row, last-col]
            ...
            \hline
              &   &   &   & - & - & - & - & 4
        \end{bNiceArray}\; , \quad
        \partial_2' = \begin{bNiceArray}{cccc}[small, first-row, last-col]
            \mathrm A & \mathrm B & \mathrm C & \mathrm D & \\
            + &   &   &   & \mathrm a \\
              & + &   &   & \mathrm b \\
              &   & + &   & \mathrm c \\
...
\begin{figure}[ht]
    ...
    \caption{2-2 and 2-3 flips implemented as sequences of merges and splits.
    The vertex added in the split step is deleted when the two edges are merged in the final transformation.
    We've shown the tetrahedra in an ``exploded'' view to help with visualization.}
    \label{fig:2-2-flip}
\end{figure}

More transformations can be obtained by combining splits and merges; figure \ref{fig:2-2-flip} shows how to perform flips in 2D and 3D.
...
Several publications have argued that two families of transformations, edge removal and multi-face removal, give superior results for increasing common mesh quality measures \cite{klingner2008aggressive}.
...
The starting mesh for multi-face removal is assumed to consist of a set of triangles that are sandwiched between a pair of vertices.
The result is a set of tetrahedra that all meet at a common central edge.
...
\begin{equation}
    \partial_1' = \left[\begin{smallmatrix}\partial_1 & I \\ & -\mathbbm{1}^*\end{smallmatrix}\right],
    \;\partial_2' = \left[\begin{smallmatrix}\partial_2 & I \\ & -\partial_1\end{smallmatrix}\right],
    \;\partial_3' = \left[\begin{smallmatrix}-I \\ \partial_2\end{smallmatrix}\right]
\end{equation}
and the boundary matrices of the iterated cone are
\begin{align}
    & \partial_1'' = \left[\begin{smallmatrix}\partial_1 & I & I & \\ & -\mathbbm{1}^* & &\textcolor{purple}{+1} \\ & & -\mathbbm{1}^* & \textcolor{purple}{-1}\end{smallmatrix}\right], \quad
    \partial_2'' = \left[\begin{smallmatrix}\partial_2 & I & I & \\ & -\partial_1 & & I \\ & & -\partial_1 & -I \\ & & & -\mathbbm{1}^*\end{smallmatrix}\right],\nonumber \\
    & \qquad \partial_3'' = \left[\begin{smallmatrix}\textcolor{orange}{-I} & \textcolor{orange}{+I} & \\ \textcolor{orange}{+\partial_2} & & \textcolor{teal}{+I} \\ & \textcolor{orange}{-\partial_2} & \textcolor{teal}{-I} \\ & & \textcolor{teal}{\partial_1}\end{smallmatrix}\right],\quad
        \partial_4'' = \left[\begin{smallmatrix} I \\ I \\ -\partial_2\end{smallmatrix}\right].\label{eq:double-cone-partial4}
\end{align}
...
\begin{figure}
    \begin{minted}[fontsize=\footnotesize,linenos,numbers=right]{python}
import numpy as np
...
We implemented part of the multi-face removal and retriangulation transformations described in \cite{misztal2009tetrahedral}.
These transformations are assumed to operate on the suspension of a set of filling triangles.
In \cite{misztal2009tetrahedral}, the filling triangles are identified via a search procedure and are then transformed into another triangulation of the same polygon.
...
\begin{figure}[ht]
    \begin{minted}[fontsize=\footnotesize]{python}
def edge_collapse(
    D: Topology, vertex_ids: int
) -> Topology:
    # Form the matrix for the initial edge collapse
    P = np.eye(D[1].shape[0], dtype=np.int8)
    P[np.ix_(vertex_ids, vertex_ids)] = \
        np.array([[1, 1], [0, 0]])

    # Remove redundant and empty cells
    E = [D[0].copy(), P @ D[1], *D[2:]]
    for k in range(1, n := len(D) - 1):
        A, B = make_reduction_matrices(E[k])
        E[k], E[k + 1] = E[k] @ A, B @ E[k + 1]

    empty_cell_ids = \
        nonzero(count_nonzero(E[-1], axis=0) <= n)
    E[-1] = np.delete(E[-1], empty_cell_ids, axis=1)
    return E
    \end{minted}
    \caption{The source code the edge collapse transformation.
    The routine \texttt{make\_reduction\_matrices} (not shown) forms the row and column operations to remove redundant cells.}
    \label{fig:edge-collapse-code}
\end{figure}

\begin{figure}[ht]
    \begin{center}
        \includegraphics[width=0.8\linewidth]{figures/edge-collapse.pdf}
    \end{center}
    \caption{Edge collapsing before (a) and after (b) in the ideal case.
    In a bad case (c) we can compute the topology but the resulting geometry is always invalid (d).}
    \label{fig:edge-collapse}
\end{figure}

Finally, we implemented the edge collapse operation used in mesh simplification \cite{gueziec1995surface}.
This procedure is error-prone enough on edge-based data structures that some authors have resorted to SMT solvers \cite{papadakis2017seam}.
The source code is shown in figure \ref{fig:edge-collapse-code}.

An edge collapse merges two vertices $v_0$, $v_1$ into one.
We implemented this in three phases.
First, we sum the rows in $\partial_1$ corresponding to $v_0$ and $v_1$ together; call the merged vertex $v^*$.
This step creates a host of degeneracies.
If we had a third vertex $u$ and edges $e_0 = \langle u, v_0\rangle,$ $e_1 = \langle u, v_1\rangle$, then after this first step we are left with two copies of the edge $\langle u, v^*\rangle$.
From a linear algebra perspective, the columns corresponding to the two copies are scalar multiples of each other.
In the second phase, we apply column operations to $\partial_1$ to remove such redundant edges and row operations to $\partial_2$ to collapse any adjacency to the redundant edges down to one remaining representative.
We then repeat this procedure at each higher dimension.
This elimination procedure is equivalent to a merge operation but applied to the dual complex.
Finally, we remove any $k$-cells with fewer than $k + 1$ faces.

The results of the edge collapse operation on a simple and a less-simple case are shown in figure \ref{fig:edge-collapse}.
The first case is straightforward and produces well-shaped triangles.
In the second case, we can compute the collapsed topology, but the resulting geometry is invalid because it introduces triangles of zero or negative area.
Mesh simplification algorithms have to detect and reject these cases \cite{gueziec1995surface}.
...
We focused here on serial algorithms to produce a simplicial mesh.
The transformations that we defined here are also applicable to hex or poly meshing.
Moreover, 3D finite element analysis often requires decomposing the mesh into subdomains for distributing to multiple processes.
The connectivity between subdomains usually does not have a simplicial or cubical structure.
We can then describe the overall topology hierarchically -- a simplicial representation inside a domains and a polytopal complex between domains.
...
The linear-algebraic representation might be able to accelerate SPR by instead bisecting cavities into smaller polytopes that only become simplicial by the end.
The branch and bound procedure over polytopes might be faster or easier to implement than its counterpart over tetrahedra.

We evaluated our code by testing that we could compute convex hulls, Delaunay triangulations, etc.
We did not give formal proofs that each operation always produces a valid end state.
For example, applying the face-split transformation assumes that a certain linear system is solvable.
Are there cases where this system has either no solution or multiple solutions but the polytope is splittable?
Are there cases where the system has a unique solution but not one that produces the desired final topology?
We found empirically that the linear system was solvable in all the cases that we tried and gave the expected results.
A future direction would be to use formal methods to verify these transformations using established results, for example on total unimodularity of boundary matrices \cite{dey2010optimal}.
...
Thanks to Matt Knepley, Tobin Isaac, Mauricio del Razo, Gilbert Bernstein, and Leila de Floriani for many helpful discussions.
